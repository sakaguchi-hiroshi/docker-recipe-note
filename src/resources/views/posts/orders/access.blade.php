@extends('layouts.app')
  @section('title', 'アクセス数ランキング')
  @section('pageCss')
    <link rel="stylesheet" href="{{ asset('/assets/css/posts/access.css')}}">
  @endsection
  @include('layouts.header.first')
  @include('layouts.header.second')
  @section('main')
    <h2 class="main-headline">アクセス数ランキング</h2>
    <form class="search-form" action="{{ route('posts.orders.access') }}" method="get">
      @csrf
    ...
    <section class="main-section">
      ...
      <div class="second-container">
        @foreach($post_recipe_rankings as $post)
          <figure class="recipe-figure">
            <p class="rank-paragraph">
              <span class="rank-number">第{{$loop->iteration}}位</span>
            </p>
            <form action="{{ route('myrecipes.show')}}" method="get">
              @csrf
      ...
      @if(empty($post_recipe_rankings->all()))
        <p class="message">条件に合うレシピがありません</p>
      @endif
    </section>
    <div class="paginate">
      {{ $post_recipe_rankings->appends(request()->input())->links() }}
    </div>
  @endsection
